          ...
          try {
            for (const [tableId, sessionMap] of pokerTables.entries()) {
              for (const [tableSessionId, table] of sessionMap.entries()) {
                ...
                if (player) {
                  socket.leave(`table-${tableId}`);
                  console.log(`❌ Player ${player.user.id} disconnected from chat room: table-${tableId}`);
                  ...
                  exitTimes.set(player.user.id, { date: Date.now(), tableId: tableId });
                  table.broadcastState();
                  break;
                }
              }
...
module.exports = { serverSocket, getFreeSits, findPlayerInAllTables };